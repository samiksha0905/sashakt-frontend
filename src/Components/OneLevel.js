...
const OneLevel = () => {
  ...
  const calculateScore = () => {
    if (startTime !== 0 && endTime !== 0) {
      ...
      if (adjustedTime < 5) {
        calculatedScore = 100; // Set score for time < 10 seconds
...
const generateCollectibles = () => {
  const collectibles = [];
  let remainingCollectibles = 5; // Number of collectibles to be added
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (elapsedTime > 20000) { // For example, if time exceeds 30 seconds
