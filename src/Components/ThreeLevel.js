...
const ThreeLevel = () => {
  ...
  const size = 7; // Set maze size to 7
...
const generateCollectibles = () => {
  const collectibles = [];
  let remainingCollectibles = 10; // Number of collectibles to be added
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (elapsedTime > 30000) { // For example, if time exceeds 30 seconds
