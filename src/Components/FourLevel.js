...
const FourLevel = () => {
  ...
  const size = 8; // Set maze size to 7
...
const generateCollectibles = () => {
  const collectibles = [];
  let remainingCollectibles = 13; // Number of collectibles to be added
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (elapsedTime > 35000) { // For example, if time exceeds 30 seconds
