...
const FiveLevel = () => {
  ...
  const size = 9; // Set maze size to 7
...
const generateCollectibles = () => {
  const collectibles = [];
  let remainingCollectibles = 15; // Number of collectibles to be added
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (elapsedTime > 40000) { // For example, if time exceeds 30 seconds
