...
const NineLevel = () => {
  ...
  const size = 13; // Set maze size to 7
  ...
  const calculateScore = () => {
    if (startTime !== 0 && endTime !== 0) {
      ...
      if (adjustedTime < 15) {
        ...
        calculatedScore = 140; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 35) {
        calculatedScore = 120; // Set score for time between 10 to 20 seconds
        ...
        calculatedScore = 35; // Set score for time between 10 to 20 seconds
      }
      else {
...
const generateCollectibles = () => {
  const collectibles = [];
  let remainingCollectibles = 25; // Number of collectibles to be added
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (elapsedTime > 60000) { // For example, if time exceeds 30 seconds
