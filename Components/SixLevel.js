...
const SixLevel = () => {
  ...
  const size = 10; // Set maze size to 7
...
const generateCollectibles = () => {
  const collectibles = [];
  let remainingCollectibles = 18; // Number of collectibles to be added
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (elapsedTime > 45000) { // For example, if time exceeds 30 seconds
