...
import GameEndPopup from './MazeGameEndPopUp';
...
const TenLevel = () => {
  ...
  const size = 15; // Set maze size to 7
  ...
  const calculateScore = () => {
    if (startTime !== 0 && endTime !== 0) {
      ...
      if (adjustedTime < 15) {
        ...
        calculatedScore = 180; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 25) {
        calculatedScore = 170; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 30) {
        calculatedScore = 150; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 35) {
        calculatedScore = 130; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 40) {
        calculatedScore = 110; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 45) {
        calculatedScore = 90; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 50) {
        calculatedScore = 75; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 55) {
        calculatedScore = 50; // Set score for time between 10 to 20 seconds
      } else if (adjustedTime < 60) {
        calculatedScore = 25; // Set score for time between 10 to 20 seconds
      }
      else {
...
const generateCollectibles = () => {
  const collectibles = [];
  let remainingCollectibles = 30; // Number of collectibles to be added
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (elapsedTime > 65000) { // For example, if time exceeds 30 seconds
